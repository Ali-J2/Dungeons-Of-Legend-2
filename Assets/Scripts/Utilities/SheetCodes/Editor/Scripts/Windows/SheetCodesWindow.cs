    ...
    public class SheetCodesWindow : EditorWindow
    {
        ...
        private void OnGUI()
        {
            if (firstFrame)
            {
                ...
                firstFrame = false;
                isShowingEnumIndex = EditorPrefs.GetBool(WindowSettings.EDITORPREFS_KEY_SHOW_ENUM_INDEX, false);
            }
        ...
        {
            if(!firstFrame)
                EditorPrefs.SetBool(WindowSettings.EDITORPREFS_KEY_SHOW_ENUM_INDEX, isShowingEnumIndex);
        ...
        private void SetDrawRangeXMin(int newValue)
        {
            ...
            if (newValue < widthDimensions.Count - 2)
            {
                ...
                selectedPage.horizontalScrollValue = newScrollValue * horizontalFactor;
            }
            else
                selectedPage.horizontalScrollValue = maxScrollValue;
        }
        ...
        private void SetDrawRangeYMin(int newValue)
        {
            ...
            if (newValue < heightDimensions.Count - 2)
            {
                ...
                selectedPage.verticalScrollValue = newScrollValue * verticalFactor;
            }
            else
                selectedPage.verticalScrollValue = maxScrollValue;
        }
        ...
        {
            //Can happen after editing a column
        ...
        private void OnCallback_CollectionEdit_Insert(object data)
        {
            ...
            newCellData.SetValue(selectedColumn.dataType.GetDefaultValue(selectedColumn.componentType, selectedColumn.enumType, false), index);
        ...
        private void DrawCollectionEdit_Buttons_AddButton(Rect buttonRect)
        {
            ...
            for (int i = 0; i < cellData.Length; i++)
                newCellData.SetValue(cellData.GetValue(i), i);
            newCellData.SetValue(selectedColumn.dataType.GetDefaultValue(selectedColumn.componentType, selectedColumn.enumType, false), cellData.Length);
            selectedCell.data = newCellData;
        }
        ...
        private void DrawLockedContent(DrawRange drawRange, CellDrawData[,] cellDrawDatas)
        {
            ...
            for (int i = 1; i < cellDrawDatas.GetLength(0); i++)
            {
                ...
                string text = sheetColumn.GetColumnText();
        ...
        private void DrawScrollbars()
        {
            ...
            if (totalHeight > visibleHeight)
            {
                ...
                selectedPage.verticalScrollValue = GUI.VerticalScrollbar(scrollbarRect, selectedPage.verticalScrollValue, visibleHeight, 0, totalHeight);
            }
            else
                selectedPage.verticalScrollValue = 0;
            ...
            if (totalWidth > visibleWidth)
            {
                ...
                selectedPage.horizontalScrollValue = GUI.HorizontalScrollbar(scrollbarRect, selectedPage.horizontalScrollValue, visibleWidth, 0, totalWidth);
            }
            else
                selectedPage.horizontalScrollValue = 0;
        }
        ...
        private void OnCallback_AddRow(SheetRow sheetRow, int insertIndex)
        {
            ...
            selectedPage.verticalScrollValue = totalHeight - visibleHeight;
            Repaint();
        }
        ...
        private void OnCallback_AddColumn(SheetColumn sheetColumn, int insertIndex)
        {
            ...
            GUIStyle guiStyle = new GUIStyle();
            int columnWidth = (int)guiStyle.CalcSize(new GUIContent(sheetColumn.GetColumnText())).x + WindowSettings.WIDTH_PADDING;
            int width = Mathf.Clamp(columnWidth, WindowSettings.MINWIDTH, WindowSettings.MAXWIDTH_INITALIZATION);
            
            if (insertIndex <= widthDimensions.Count)
                widthDimensions.Insert(insertIndex + 1, width);
            else
                widthDimensions.Add(width);
            selectedPage.horizontalScrollValue = totalWidth - visibleWidth;
            Repaint();
        }
        ...
        private int GetStartHeightIndex()
        {
            ...
            int maxScrollValue = totalHeight - visibleHeight;
            float verticalFactor = selectedPage.verticalScrollValue / maxScrollValue;
            float scrollValue = heightOverflow * verticalFactor + selectedPage.verticalScrollValue;
        ...
        private int GetStartWidthIndex()
        {
            ...
            int maxScrollValue = totalWidth - visibleWidth;
            float horizontalFactor = selectedPage.horizontalScrollValue / maxScrollValue;
            float scrollValue = widthOverFlow * (selectedPage.horizontalScrollValue / maxScrollValue) + selectedPage.horizontalScrollValue;
        ...
        {
            if (sheetColumn.isCollection)
                return ShowGUILayout_Collection(drawRect, sheetColumn, (Array)data, selected);
            else
        ...
        private object ShowGUILayout_Single(Rect drawRect, SheetColumn sheetColumn, object data)
        {
            switch (sheetColumn.dataType)
            {
                ...
                case SheetDataType.Color:
                    return ShowGUILayout_Color(drawRect, (Color)data);
                ...
                case SheetDataType.Enum:
                    {
                        if (sheetColumn.enumFlags)
                            return ShowGUILayout_EnumFlags(drawRect, (Enum)data);
                        else
                            return ShowGUILayout_Enum(drawRect, (Enum)data);
                    }
                case SheetDataType.Component:
                    return ShowGUILayout_Component(drawRect, sheetColumn.componentType, (UnityEngine.Object)data);
            }
            throw new MissingReferenceException(string.Format(Localization.EXCEPTION_SHOWGUILAYOUT, sheetColumn.dataType));
        }

        private Array ShowGUILayout_Collection(Rect drawRect, SheetColumn sheetColumn, Array data, bool selected)
        {
            string result = string.Empty;
            if (sheetColumn.dataType == SheetDataType.Enum && sheetColumn.enumFlags)
            {
                for (int i = 0; i < data.Length; i++)
                {
                    ...
                    result += i + ":";
                    if (arrayData.ToString() == "-1")
                        result += "Everything";
                    else if (arrayData.ToString() == "0")
                        result += "Nothing";
                    else
                        result += arrayData.ToString();
            ...
            else
            {
                for (int i = 0; i < data.Length; i++)
                {
                    ...
                    object arrayData = data.GetValue(i);
                    result += i + ":";
                    result += arrayData != null && !arrayData.Equals(null) ? arrayData.ToString() : Localization.NULLOBJECT_TO_STRING;
                }
            }
        ...
        private Color ShowGUILayout_Color(Rect drawRect, Color color)
        {
            drawRect.height = EditorGUI.GetPropertyHeight(SerializedPropertyType.Color, new GUIContent(""));
            return EditorGUI.ColorField(drawRect, color);
        }
        ...
        private Enum ShowGUILayout_Enum(Rect drawRect, Enum data)
        {
            drawRect.height = EditorGUI.GetPropertyHeight(SerializedPropertyType.Enum, new GUIContent(""));
            return EditorGUI.EnumPopup(drawRect, data);
        }

        private Enum ShowGUILayout_EnumFlags(Rect drawRect, Enum data)
        {
            drawRect.height = EditorGUI.GetPropertyHeight(SerializedPropertyType.Enum, new GUIContent(""));
            return EditorGUI.EnumFlagsField(drawRect, data);
        }

        private UnityEngine.Object ShowGUILayout_Component(Rect drawRect, Type componentType, UnityEngine.Object data)
        {
            drawRect.height = EditorGUI.GetPropertyHeight(SerializedPropertyType.Enum, new GUIContent(""));
            return EditorGUI.ObjectField(drawRect, data, componentType, false);
        }
        #endregion
        ...
        private Vector2 GetMinSize_Collection(SheetColumn sheetColumn, object data)
        {
            switch (sheetColumn.dataType)
            {
                ...
                case SheetDataType.Color:
                    return GetMinSize_Collection_Color((Color[])data);
                case SheetDataType.Reference:
                    return GetMinSize_Collection_String((string[])data);
                case SheetDataType.Enum:
                    return GetMinSize_Collection_Enum((Array)data);
                case SheetDataType.Component:
                    return GetMinSize_Collection_Component((UnityEngine.Object[])data);
            }
        ...
        private Vector2 GetMinSize_Single(SheetColumn sheetColumn, object data)
        {
            switch (sheetColumn.dataType)
            {
                ...
                //Never show overflow indicators for Color, Enum, Reference and Component singles
                case SheetDataType.Color:
                    return new Vector2();
                case SheetDataType.Enum:
                    return new Vector2();
        ...
        private Vector2 GetMinSize_Collection_Color(Color[] datas)
        {
            float minHeight = datas.Length * WindowSettings.ONE_LINE_HEIGHT + WindowSettings.HEIGHT_PADDING;
            float minWidth = 0;
            if(datas.Length > 0)
                minWidth = Math.Max(minWidth, GetMinSize_Single_String("RGBA(0.000, 0.000, 0.000, 0.000)").x);
            return new Vector2(minWidth, minHeight);
        }

        private Vector2 GetMinSize_Collection_Enum(Array datas)
        {
            float minHeight = datas.Length * WindowSettings.ONE_LINE_HEIGHT + WindowSettings.HEIGHT_PADDING;
            float minWidth = 0;
            for (int i = 0; i < datas.Length; i++)
                minWidth = Math.Max(minWidth, GetMinSize_Single_String(datas.GetValue(i).ToString()).x);
            return new Vector2(minWidth, minHeight);
        }
        ...
        private Vector2 GetMinSize_Collection_Component(UnityEngine.Object[] datas)
        {
            float minHeight = datas.Length * WindowSettings.ONE_LINE_HEIGHT + WindowSettings.HEIGHT_PADDING;
            float minWidth = 0;
            foreach (UnityEngine.Object data in datas)
            {
                string content = data != null ? data.ToString() : Localization.NULLOBJECT_TO_STRING;
                minWidth = Math.Max(minWidth, GetMinSize_Single_String(content).x);
            }
            return new Vector2(minWidth, minHeight);
        }
        #endregion
